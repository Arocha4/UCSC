...
public static void main(String[] args) throws IOException {
        ...
        for (int m = 1; m < J; m++) {
			...
			// starts printed along with singular or plural sentenses
			Trace.println("******************************************************");
            ...
            while (pro[0].isEmpty() || pro[0].length() != J || ((Job) pro[0].peek()).getFinish() == -1) {
                ...
                for (int i = 1; i < pro.length; i++) {
                    if (!pro[i].isEmpty() && ((Job) pro[i].peek()).getFinish() == -1)
                        ((Job) pro[i].peek()).computeFinishTime(time);
                } //computes the finishing time

                //computing is done. print out job process along with time
            ...
            //calculate avg & total time and total wait time
            totalWait = 0;
            ...
            //print to ex.trc and ex.rpt output files in directory
